# GEMINI.md — AI Agent Instructions for Gemini CLI
# This file is read automatically by Gemini CLI at session start.
...
This project uses Lefthook pre-commit hooks that run automatically on `git commit`.
If a commit is rejected, the error output tells you exactly what's wrong.
Read the errors, fix them, and retry the commit.

Checks that run on every commit:
- Formatting (Prettier)
- Dead code (Knip)
- Dependency versions (Syncpack)
- Architecture boundaries (ESLint)
...
Valid scopes: package names + `deps`, `ci`, `release`
